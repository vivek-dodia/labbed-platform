...
export default function TopologyEditorPage() {
  ...
  const handleSave = useCallback(async () => {
    setSaving(true);
    try {
      const updated = await api.put<TemplateResponse>(
        `/api/v1/templates/${id}`,
        { definition }
      );
      setTemplate(updated);
  ...
  const handleDeleteFile = useCallback(
    async (fileId: string) => {
      await api.del(`/api/v1/templates/${id}/files/${fileId}`);
      setTemplate((prev) =>
        prev
    ...
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#79f673", color: "#000000", fontFamily: "'Manrope', sans-serif" }}>
      ...
      <div style={{ flexGrow: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <nav style={{ height: "48px", borderBottom: "1px solid #000000", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", height: "100%" }}>
            ...
            <span style={{ ...navItemStyle, opacity: 0.5, cursor: "default" }}>{template.name}</span>
          </div>
          <div style={{ display: "flex", height: "100%" }}>
      ...
      {showFileEdit && (
        <div onClick={() => setShowFileEdit(null)} style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: "#111", border: "1px solid rgba(121,246,115,0.2)", borderRadius: 6, maxWidth: 1200, width: "95%", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            {/* Header */}
              ...
              <div style={{ display: "flex", gap: "0.75rem" }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    await api.patch(`/api/v1/templates/${id}/files/${showFileEdit.uuid}`, { content: editContent });
